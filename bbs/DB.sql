...
DROP TABLE IF EXISTS ggh_bbs.category;
...
CREATE TABLE `ggh_bbs`.`board` (
  ...
  `b_createdate` TIMESTAMP NOT NULL,
  `b_hit` INT NOT NULL,
  PRIMARY KEY (`b_id`),
  INDEX `fk_user_idx` (`b_writer` ASC),
  INDEX `fk_cate_idx` (`b_category` ASC),
  CONSTRAINT `fk_user` FOREIGN KEY (`b_writer`) REFERENCES `ggh_bbs`.`user` (`u_no`)
    ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `fk_cate` FOREIGN KEY (`b_category`) REFERENCES `ggh_bbs`.`category` (`c_id`)
    ON DELETE NO ACTION ON UPDATE CASCADE);
